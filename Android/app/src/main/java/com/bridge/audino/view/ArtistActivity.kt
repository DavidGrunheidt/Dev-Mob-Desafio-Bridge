...
class ArtistActivity : TrackListActivity(), OnBackPressedListener, AlbumClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mBinding.artistAlbumList.adapter = AlbumListAdapter(this)

        mViewModel.getFullArtist().observe(this, Observer<FullArtist> {
            fillInArtist(it)
            (mBinding.artistAlbumList.adapter as AlbumListAdapter).setAlbumList(it.albums)
        })
    ...
    override fun onAlbumClicked(album: FullAlbum) {
        val intent = Intent(this, AlbumActivity::class.java).apply {
            putExtra(AlbumViewModel.ARTIST_ID, idArtist)
            putExtra(AlbumViewModel.ALBUM_ID, album.id)
        }
