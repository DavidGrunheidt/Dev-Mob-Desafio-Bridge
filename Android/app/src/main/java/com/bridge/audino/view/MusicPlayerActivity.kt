...
class MusicPlayerActivity : AppCompatActivity(), OnBackPressedListener, MusicPlayerFavoriteListener {
    ...
    private fun updateRepeatStatus(status: Int) {
        when (status) {
            REPEAT_OFF -> {
                mBinding.repeatIb.setImageResource(R.drawable.ic_repeat)
                mBinding.repeatIb.imageTintList = ContextCompat.getColorStateList(this, R.color.off_white)
            }
            REPEAT_ON -> mBinding.repeatIb.imageTintList = ContextCompat.getColorStateList(this, R.color.blue)
            REPEAT_ONE -> mBinding.repeatIb.setImageResource(R.drawable.ic_repeat_one)
        }
    }
